...
class LoginPage extends RunnerPage 
{
	...
	public function process()
	{
		...
		if( $this->fb_error_code || $this->fb_error_reason )
		{
			echo "<script>window.close();</script>";
			return;
		}
		
		if( $this->fb_state || $this->fb_code ) 
		{
			if( $this->LogInFromFacebook() )
				$this->redirectAfterFBlogin();
			else
				echo "<script>window.close();</script>";
			return;
		}
	...
	protected function getSelectSQL( $skipPasswordCheck, $strUsername, $strPassword, $loginSet, $cipherer )
	{
		...
		{	
			...
			if( $loginSet )
				...
				$activateWhere = " and ".$this->connection->addFieldWrappers("USR_COST_BY_HOUR")."=1";
	...
	public function Logout($redirectToLogin = false) 
	{
		...
		$fbObj = new facebookWrapper();
		$cookieName = $fbObj->getCookieName();
		if( $cookieName ) 
	 ...
	 *
	 */
	protected function redirectAfterFBlogin()
	{
		//redirect after success login
		if( $this->myurl ) 
		{
			echo"<script>window.opener.location.href = '".$this->myurl."'; window.close();</script>";
		} 
		else 
		{
	...
	/** 
	 * @param String cUserNameField
	 * @param String fbUsername
	 * @return Array
	 */
	public function getLoginDataByFbUserId( $cUserNameField, $fbUsername )
	{
		$strSQL = "select * from ".$this->connection->addTableWrappers("USERS")
			." where ".$this->connection->addFieldWrappers($cUserNameField)."='".$fbUsername."'";
		
		return $this->connection->query( $strSQL )->fetchAssoc();
	}
	...
	{	
		global $cUserNameField, $cDisplayNameField, $cPasswordField, $cCharset;
		
		$fbme = $this->fbObj->FBgetUserInfo();
		if( $fbme )
		{
			$strUsername = "fb".(string)$fbme["id"];
			$strDisplayName = runner_convert_encoding((string)$fbme["name"], $cCharset, 'UTF-8');
			//is in DB
			$data = $this->getLoginDataByFbUserId( $cUserNameField, $strUsername );
			if( $data )
			{
				$pUsername = $data[ $cUserNameField ];
				$pDisplayUsername = $strDisplayName != '' ? $strDisplayName : $data[ $cDisplayNameField ];
				$pPassword = $data[ $cPasswordField ];
				$this->fromFacebook = true;
				$_SESSION["fromFacebook"] = true;
			}
			else 
			{
				//insert new account from facebook
				$strPassword = generatePassword(10);
						
				$values["USR_UID"] = $strUsername;
				$values["USR_PASSWORD"] = $strPassword;
				if( "USR_FIRSTNAME" != "USR_UID" && "USR_FIRSTNAME" != '' )
					$values["USR_FIRSTNAME"] = $strDisplayName;
			
						$values["USR_COST_BY_HOUR"] = 1;
				...
				if( $retval ) 
				{
					...
					$pDisplayUsername = $strDisplayName;
					$this->fromFacebook = true;
					$_SESSION["fromFacebook"] = true;
				}
			}
			...
			if( $this->callBeforeLoginEvent() )
			{
				AfterFBLogIn($pUsername, $pPassword, $this);
				return true;				
			}
		}
		return false;
