...
require_once("include/PMT_SPREADSUB_variables.php");
...
$layout->version = 3;
	$layout->bootstrapTheme = "default";
		$layout->customCssPageName = "PMT_SPREADSUB_detailspreview";
...
$page_layouts["PMT_SPREADSUB_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	//	APP_UID2 - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("APP_UID2", $data, $keylink);
			$row["APP_UID2_value"] = $value;
			$format = $pSet->getViewFormat("APP_UID2");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("APP_UID2")))
				$class = ' rnr-field-number';
			$row["APP_UID2_class"] = $class;
		$rowinfo[] = $row;
		if ($b) {
			$rowinfo2[] = $row;
			$b = false;
...
$xt->load_template(GetTemplateName("PMT_SPREADSUB", "detailspreview"));
