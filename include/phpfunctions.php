...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="APPLICATION" && $field=="APP_UID")
	{
		
$dat = unserialize($data["APP_DATA"]);
$num = $dat["INDEX"] + 1;
$value = "<a href =";
$value .= "https://dev.bpmboxes.com/sysworkflow/en/neoclassic/cases/open?APP_UID=".$data["APP_UID"]."&DEL_INDEX=".$num."&action=todo";
$value .= ">link</a>";;
		return $value;
	}
				if($table=="APPLICATION" && $field=="APP_DATA")
	{
		$a = $data["APP_DATA"];
$d = unserialize($a);
...
$value = "";//"<pre>".var_export($d,true)."</pre>";;
		return $value;
	}
				if($table=="APP_CACHE_VIEW" && $field=="APP_UID")
	{
		$value = "<a href =";
$value .= "/sysworkflow/en/neoclassic/cases/open?APP_UID=".$data["APP_UID"]."&DEL_INDEX=".$data["DEL_INDEX"]."&action=todo";
$value .= ">link</a>";;
		return $value;
	}
				if($table=="PMT_COMMAND" && $field=="cmd")
	{
		$value = "<pre><code class='php'>".$data["cmd"]."<code></pre>";;
		return $value;
	}
				if($table=="APP_CACHE_VIEW_INDEX" && $field=="APP_UID")
	{
		$value = "<a href =";
$value .= "/sysworkflow/en/neoclassic/cases/open?APP_UID=".$data["APP_UID"]."&DEL_INDEX=".$data["DEL_INDEX"]."&action=todo";
$value .= ">link</a>";;
		return $value;
				...
				if($table=="APP_CACHE_VIEW_DRAFT" && $field=="APP_UID")
	{
		$value = "<a href =";
$value .= "/sysworkflow/en/neoclassic/cases/open?APP_UID=".$data["APP_UID"]."&DEL_INDEX=".$data["DEL_INDEX"]."&action=todo";
$value .= ">link</a>";;
		return $value;
				...
				if($table=="APP_CACHE_VIEW_PARTICIPATE" && $field=="APP_UID")
	{
		$value = "<a href =";
$value .= "/sysworkflow/en/neoclassic/cases/open?APP_UID=".$data["APP_UID"]."&DEL_INDEX=".$data["DEL_INDEX"]."&action=todo";
$value .= ">link</a>";;
		return $value;
				...
				if($table=="APP_CACHE_VIEW_PAUSE" && $field=="APP_UID")
	{
		$value = "<a href =";
$value .= "/sysworkflow/en/neoclassic/cases/open?APP_UID=".$data["APP_UID"]."&DEL_INDEX=".$data["DEL_INDEX"]."&action=todo";
$value .= ">link</a>";;
		return $value;
				...
				if($table=="APPLICATION Chart" && $field=="APP_UID")
	{
		
$dat = unserialize($data["APP_DATA"]);
$num = $dat["INDEX"] + 1;
$value = "<a href =";
$value .= "/sysworkflow/en/neoclassic/cases/open?APP_UID=".$data["APP_UID"]."&DEL_INDEX=".$num."&action=todo";
$value .= ">link</a>";;
		return $value;
	}
				if($table=="APPLICATION Chart" && $field=="APP_DATA")
	{
		$a = $data["APP_DATA"];
$d = unserialize($a);
foreach($d as $key => $val){
...
$value = "";//"<pre>".var_export($d,true)."</pre>";;
		return $value;
				...
				if($table=="APPLICATION Report" && $field=="APP_UID")
	{
...
$value .= ">link</a>";;
	...
	if(!$table)
		$table=$strTableName;
	$value=$data[$field];
	if((strpos($table,"ppmain") || $table == "ppmain") && $field=="but") 
	{
...
onclick=\"javascript: document.forms.xclick.item_name.value='".$data["Title"]."'; 
