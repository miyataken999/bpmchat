...
require_once("include/modx_site_templates_variables.php");
...
$layout->version = 3;
	$layout->bootstrapTheme = "default";
		$layout->customCssPageName = "modx_site_templates_detailspreview";
...
$page_layouts["modx_site_templates_detailspreview"] = $layout;
...
if($mastertable == "modx_site_content")
{
	...
	if( $formattedValue == "null" )
		$where .= $pageObject->getFieldSQLDecrypt("id") . " is null";
	else
		$where .= $pageObject->getFieldSQLDecrypt("id") . "=" . $formattedValue;
}
...
if($rowcount) 
{
	...
	//	templatename - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("templatename", $data, $keylink);
			$row["templatename_value"] = $value;
			$format = $pSet->getViewFormat("templatename");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("templatename")))
				$class = ' rnr-field-number';
			$row["templatename_class"] = $class;
	//	description - 
			$viewContainer->recId = $recordsCounter;
	...
	//	icon - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("icon", $data, $keylink);
			$row["icon_value"] = $value;
			$format = $pSet->getViewFormat("icon");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("icon")))
				$class = ' rnr-field-number';
			$row["icon_class"] = $class;
	//	template_type - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("template_type", $data, $keylink);
			$row["template_type_value"] = $value;
			$format = $pSet->getViewFormat("template_type");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("template_type")))
				$class = ' rnr-field-number';
			$row["template_type_class"] = $class;
	//	content - HTML
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("modx_site_templates", "detailspreview"));
