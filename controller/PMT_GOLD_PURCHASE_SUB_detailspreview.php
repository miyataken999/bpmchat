...
require_once("include/PMT_GOLD_PURCHASE_SUB_variables.php");
...
$layout->version = 3;
	$layout->bootstrapTheme = "default";
		$layout->customCssPageName = "PMT_GOLD_PURCHASE_SUB_detailspreview";
...
$page_layouts["PMT_GOLD_PURCHASE_SUB_detailspreview"] = $layout;
...
if($mastertable == "PMT_GOLD_PURCHASE")
{
	$where = "";
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["APP_UIDSUB"]));
	
	
	//	APP_UIDSUB - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("APP_UIDSUB", $data, $keylink);
			$row["APP_UIDSUB_value"] = $value;
			$format = $pSet->getViewFormat("APP_UIDSUB");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("APP_UIDSUB")))
				$class = ' rnr-field-number';
			$row["APP_UIDSUB_class"] = $class;
	//	APP_NUMBER - 
			$viewContainer->recId = $recordsCounter;
	...
	//	MONTHLY_REPORT - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("MONTHLY_REPORT", $data, $keylink);
			$row["MONTHLY_REPORT_value"] = $value;
			$format = $pSet->getViewFormat("MONTHLY_REPORT");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("MONTHLY_REPORT")))
				$class = ' rnr-field-number';
			$row["MONTHLY_REPORT_class"] = $class;
		$rowinfo[] = $row;
		if ($b) {
			$rowinfo2[] = $row;
			$b = false;
...
$xt->load_template(GetTemplateName("PMT_GOLD_PURCHASE_SUB", "detailspreview"));
